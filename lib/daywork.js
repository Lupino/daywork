...
function hashedPassword(rawPassword) {
...
export default class extends Object {
  ...
  changePasswd(pwds, callback) {
    ...
    if (pwds.passwd != pwds.repasswd) {
      return callback('password no match.');
    }
    var oldHash = hashedPassword(pwds.oldPasswd);
    var hash = hashedPassword(pwds.passwd);
    User.findOneAndUpdate({ userId: pwds.userId, passwd: oldHash },
                          null,
      ...
      if (req.url.match(/^\/(js|css|img|favicon|logout)$/)) {
        ...
        if (type === 'refresh_token') {
          ...
          this.doAuth(body.userName, body.passwd, (err, user) => {
            if (err || !user) {
              return res.json({ err: 403, msg: '用户名或密码错误' });
            }
  ...
  publishJob(jobId, callback) {
    ...
    Job.findOneAndUpdate(query, null, {status: 'Publish'}, (err, job) => callback(err, job));
  }

  finishJob(jobId, callback) {
    let query = { jobId: jobId, status: 'Publish' };
    Job.findOneAndUpdate(query, null, {status: 'Finish'}, (err, job) => callback(err, job));
  }

  deleteJob(jobId, callback) { // you can set job deleted on Draft or Finish
    let query = { jobId: jobId, status: { $in: [ 'Draft', 'Finish' ] }};
    Job.findOneAndUpdate(query, null, {status: 'Deleted'}, (err, job) => callback(err, job));
  }
  ...
  leaveMyJob(userId, jobId, callback) {
    ...
    MyJob.findOneAndUpdate(query, null, {status: 'Leave'}, (err, myJob) => callback(err, myJob));
  }

  finishMyJob(userId, jobId, callback) {
    let query = { userId: userId, jobId: jobId, status: 'Join' };
    MyJob.findOneAndUpdate(query, null, {status: 'Finish'}, (err, myJob) => callback(err, myJob));
  }
  ...
  updateRecord(record, callback) {
    ...
    WorkRecord.findOneAndUpdate(query, null, {status: record.status},
